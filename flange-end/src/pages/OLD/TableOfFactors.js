...
class TableOfFactor extends Component {
    reqest_number="";
    constructor(props) {
        super(props);
        this.state = {
        ...
        this.GetTableOfFactor = this.GetTableOfFactor.bind(this);
    ...
    async GetTableOfFactor() {
        const send_obj = {
            'method': 'get',
            'url': this.props.server+'getdata/gettableoffactor',
            'params': { test: 'test' },
        }
        await axios(send_obj)
            .then((response) => {
                this.setState({
                    TableOfFactor: (response.data),
                });
    ...
    CreateRow() {
        let counter = 0;
        for (let item of this.state.TableOfFactor) {
